...
$(function() {
  ...
  $('#contactFormt').submit(function(event) {
    ...
    if (formValid && captcha.length==6) {
      ...
      formData.append('namet', namet);
      // добавить в formData значение 'email'=значение_поля_email
      formData.append('emailt', emailt);
      ...
      formData.append('captchat', captcha);
      ...
      $.ajax({
        ...
        success : function(data){
          ...
          if ($data.result == "success") {
            ...
            // обновляем форму:
            $('#contactFormt input,textarea').val('');
            $('#contactFormt input,textarea').css('background-color','#fff')
            // выводим новый код капча
            $('#img-captchat').attr('src','/feedback/captcha.php?id='+Math.random()+'');
